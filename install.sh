...
chmod 0755 "$GATEWAY"
# Run a clean bash without profile/rc processing. We explicitly source the
# user's .bashrc with nounset disabled, then pass Python/gateway as positional
# parameters to the command string. This avoids Bash interpreting paths or
# separators as invocation options.
...
exec /bin/bash --noprofile --norc -c 'set +u; if [[ -f "\$1" ]]; then source "\$1" || true; fi; exec "\$2" "\$3"' _ $Q_BASHRC "$PY" "$GATEWAY"
EOF
